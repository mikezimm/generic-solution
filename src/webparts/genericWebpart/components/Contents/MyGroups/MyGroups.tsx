 ...
 */
    ...
    public render(): React.ReactElement<IMyGroupsProps> {
        ...
        let showThisTitle = null;
        if ( this.props.title !== 'showGroupTitle' ) {
          showThisTitle = this.props.title;
        } else {
          if ( this.state.indexSelectedKey === SiteAdminGroupName || this.state.indexSelectedKey === GuestsGroupName ) {
            showThisTitle = 'Showing members of ' +  this.state.indexSelectedKey;
          } else { showThisTitle = 'Showing members of...'; }
        }
        ...
        let groupPivot = <div><Pivot
          ...
          >
            { this.state.myGroups.sortedGroups.map(( thisGroup ) => { //_renderAdminsIcon
                let headerText = this.props.minAdminGuestIcons !== true || [ SiteAdminGroupName, GuestsGroupName ].indexOf( thisGroup.Title ) < 0 ? thisGroup.Title : '';
                return (<PivotItem headerText={ headerText } itemKey={thisGroup.Title} key={thisGroup.Title} itemIcon={ thisGroup.groupProps.icon } /> );
            })}
          </Pivot></div>;
        ...
        if ( isLoaded !== true || !selectedGroup ) {
          ...
          if ( this.props.personaSize === PersonaSize.size10 ) {
            size = PersonaSize.size10;
            iconSize = 12;

          } else if ( this.props.personaSize === PersonaSize.size16 ) {
            size = PersonaSize.size16;
            iconSize = 14;

          } else if ( this.props.personaSize === PersonaSize.size28 ) {
            size = PersonaSize.size28;
            iconSize = 14;

          } else if ( this.props.personaSize === PersonaSize.size32 ) {
 ...
 */
        ...
        return (
          <div className={styles.directory} style={{ width: this.props.width, maxWidth: this.props.maxWidth }}>
            { webpartTitle }
...
/*        users:
  ...
  private _selectedIndex(item?: PivotItem, ev?: React.MouseEvent<HTMLElement>) {
    ...
    if (ev.ctrlKey) {
      if ( clickedGroup === SiteAdminGroupName && this.state.isSiteAdmin ) {
        window.open( this.props.context.pageContext.site.absoluteUrl + '/_layouts/15/user.aspx', '_blank');

      } if ( clickedGroup === GuestsGroupName ) {
        // do not go any where
      } else {
